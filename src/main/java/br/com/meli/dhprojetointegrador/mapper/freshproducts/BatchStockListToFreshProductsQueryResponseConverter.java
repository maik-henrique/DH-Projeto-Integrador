...
@Component
public class BatchStockListToFreshProductsQueryResponseConverter
                extends AbstractConverter<BatchStockCollection, FreshProductsQueriedResponse> {
        @Override
        protected FreshProductsQueriedResponse convert(BatchStockCollection source) {
                ...
                BatchStock batchStock = batchStockList.stream().findAny()
        ...
        private List<BatchStockResponse> extractBatchStock(List<BatchStock> source) {
                return source
                                .stream().map(batchStock -> BatchStockResponse.builder()
                                                ...
                                                .sectionCode(String.valueOf(
                                                                batchStock.getInboundOrder().getSection().getId()))
                                                .build())
